(ns braze.main
  "Kotodama WASM entrypoint for the Braze clean-room actor (L4) — Clojure port.
  ...
  facts (`braze.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Campaign"  :plural "campaigns"  :id-prefix "braze_cam"
    :fields [:name :channel :status :budget]   :required [:name :channel]
    :coerce {:budget :float}          :refs {}}
   {:entity "Audience"  :plural "audiences"  :id-prefix "braze_aud"
    :fields [:name :size :definition]          :required [:name :size]
    :coerce {:size :int}              :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "braze_eve"
    :fields [:profileId :name :properties :occurredAt] :required [:name :properties]
    :coerce {}                       :refs {:profileId "Profile"}}
   {:entity "Profile"   :plural "profiles"   :id-prefix "braze_pro"
    :fields [:email :externalId :traits]       :required [:email :traits]
    :coerce {}                       :refs {}}
   {:entity "Message"   :plural "messages"   :id-prefix "braze_mes"
    :fields [:campaignId :profileId :channel :status] :required [:channel :status]
    :coerce {}                       :refs {:campaignId "Campaign" :profileId "Profile"}}
   {:entity "Funnel"    :plural "funnels"    :id-prefix "braze_fun"
    :fields [:name :steps :conversionRate]     :required [:name :steps]
    :coerce {:conversionRate :float} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"braze.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "braze-compat" :tier tier :entities entities} 200])
